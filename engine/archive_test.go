...
func TestToArchive_RelaxationConversion(t *testing.T) {
	result := &RunResult{
		Outcome: OutcomePassed,
		Steps: []StepResult{
			{
				...
				Relaxations: []RelaxationRecord{
					{
						...
						InputRef:       "step1.departureDate",
						Reason:         "resolution_exhausted",
						Depth:          1,
					},
					{
						ConstraintName: "carrier_preference",
						InputRef:       "step1.carrier",
						Reason:         "step_failed",
						Depth:          2,
					},
				},
	...
	r0 := a.Steps[0].Relaxations[0]
	assert.Equal(t, "date_preference", r0.ConstraintName)
	assert.Equal(t, "step1.departureDate", r0.InputRef)
	assert.Equal(t, "resolution_exhausted", r0.Reason)
	assert.Equal(t, 1, r0.Depth)

	r1 := a.Steps[0].Relaxations[1]
	assert.Equal(t, "carrier_preference", r1.ConstraintName)
	assert.Equal(t, "step_failed", r1.Reason)
	assert.Equal(t, 2, r1.Depth)
}

func TestToArchive_RelaxedResolutionConversion(t *testing.T) {
	result := &RunResult{
		Outcome: OutcomePassed,
		Steps: []StepResult{
			{
				...
				Resolutions: []ValueResolution{
					{
						...
						PoolIndex:         -1,
						Relaxed:           true,
						RelaxedConstraint: "airport_preference",
					},
				},
			},
		},
	...
	assert.Equal(t, "airport_preference", r.RelaxedConstraint)
	require.NotNil(t, r.ConstraintOK)
	assert.False(t, *r.ConstraintOK)
	...
	lr := loaded.Steps[0].Resolutions[0]
	assert.True(t, lr.Relaxed)
	assert.Equal(t, "airport_preference", lr.RelaxedConstraint)
}

func TestToArchive_FilterRelaxedSelectionConversion(t *testing.T) {
	result := &RunResult{
		Outcome: OutcomePassed,
		Steps: []StepResult{
			{
	...
	require.Len(t, a.Steps[0].Selections, 1)
	sel := a.Steps[0].Selections[0]
	assert.True(t, sel.FilterRelaxed)
	assert.Equal(t, "carrier == 'DL'", sel.FilterExpr)
	...
	lsel := loaded.Steps[0].Selections[0]
	assert.True(t, lsel.FilterRelaxed)
}

func TestToArchive_NoRelaxations(t *testing.T) {
	// Without relaxations, field should be omitted in JSON
	result := &RunResult{
		Outcome: OutcomePassed,
