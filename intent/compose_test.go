...
func TestBuildOutputMap(t *testing.T) {
	...
	assert.Equal(t, "book.itineraryId", m["itineraryId"])
	assert.Equal(t, "book.offerRef", m["offerRef"])
...
func TestAutoWirePlaceholders(t *testing.T) {
	...
	assert.Nil(t, step.Values["itineraryId"].Default)

	// Auto-wired from output map.
	assert.Equal(t, "addOffer.offerRef", step.Values["offerRef"].From)
	assert.Nil(t, step.Values["offerRef"].Default)
...
func TestAutoWirePlaceholders_ExplicitWireOverride(t *testing.T) {
	...
	outputMap := map[string]string{
		"itineraryId": "autoStep.itineraryId", // would be auto-wired
	}

	wire := map[string]string{
		"itineraryId": "explicitStep.itineraryId", // explicit override wins
	}

	autoWirePlaceholders(sub, outputMap, wire, g)

	step := sub.Execution.Steps[0]
	assert.Equal(t, "explicitStep.itineraryId", step.Values["itineraryId"].From)
}
...
func TestMergeCleanup(t *testing.T) {
	...
	sub := &plan.Plan{
		Execution: plan.Execution{
			Cleanup: []plan.CleanupStep{
				{Node: "ignoreItinerary", RunOn: "always"}, // duplicate — should be deduped
	...
	assert.Equal(t, "ignoreItinerary", parent.Execution.Cleanup[0].Node)
...
func TestComposeWorkflowTemplate_WithAddon(t *testing.T) {
	...
	// Check auto-wiring: itineraryId in addon1 should be wired to book.itineraryId.
	addon1 := p.Execution.Steps[2]
	assert.Equal(t, "book.itineraryId", addon1.Values["itineraryId"].From)
	assert.Nil(t, addon1.Values["itineraryId"].Default)
...
func TestResolveAfterWire(t *testing.T) {
	tests := []struct {
		...
		{
			name:         "no $after refs",
			wire:         map[string]string{"itineraryId": "book.itineraryId"},
			matchedAfter: "priceOffer",
			expected:     map[string]string{"itineraryId": "book.itineraryId"},
		},
		{
			name:         "$after substitution",
			wire:         map[string]string{"offerListIdentifier": "$after.offerRef"},
			matchedAfter: "priceOfferByRef",
			expected:     map[string]string{"offerListIdentifier": "priceOfferByRef.offerRef"},
		},
		{
...
func TestComposeWorkflowTemplate_AfterWireSubstitution(t *testing.T) {
	g := buildComposeTestGraph()
	// Add an offerRef output to the book node for wire testing.
	g.Nodes["book"].Outputs = append(g.Nodes["book"].Outputs,
		graph.Output{Name: "offerRef", Type: "string"})
	...
	addon1 := p.Execution.Steps[2]
	// $after.offerRef resolved to book.offerRef.
	assert.Equal(t, "book.offerRef", addon1.Values["itineraryId"].From)
...
func TestComposeWithSlots_BasicReplacement(t *testing.T) {
	...
	// Base has: createItinerary, [trip-search], addTraveler, [payment], finalStep
	...
	// Verify step order.
	assert.Equal(t, "createItinerary", p.Execution.Steps[0].Node)
...
func TestComposeWithSlots_CrossSlotFromRef(t *testing.T) {
	...
	// CashPayment has a from: createItinerary.itineraryId — this is a cross-slot
	// ref to a base template step. fixFromDependencies should add createItinerary
	...
	assert.Contains(t, cashStep.DependsOn, "createItinerary",
...
func TestComposeWithSlots_OptionBMultiStep(t *testing.T) {
	...
	// Total: createItinerary + (search, searchLeg2, book) + addTraveler + addCashPayment + finalStep = 7
	require.Len(t, p.Execution.Steps, 7)
...
func TestComposeWithSlots_PartialChoices(t *testing.T) {
	...
	// OptionB (3 steps) + CashPayment (1 step) + createItinerary + addTraveler + finalStep = 7
	require.Len(t, p.Execution.Steps, 7)
}
...
func TestEnsureFromDeps_AddsFromRefDeps(t *testing.T) {
	...
	// commit should gain createItinerary dep (not just addOffer).
...
func buildSlotTestGraph() *graph.Graph {
	...
	g.Nodes["createItinerary"] = &graph.Node{
		Name: "createItinerary", Adapter: "createItinerary",
