			...
			Steps: []plan.Step{
				{
					...
					Node: "searchSeatMap",
					Values: map[string]plan.StepValue{
						"itineraryId":          {Default: "PLACEHOLDER"},
						"offerRef": {Default: "PLACEHOLDER"},
...
func TestFindComposedWorkflow(t *testing.T) {
	g := &graph.Graph{
		Workflows: []graph.Workflow{
			{Name: "Full Booking", Template: "plans/booking.yaml"},
	...
	t.Run("exact match", func(t *testing.T) {
		wf, found := FindComposedWorkflow(g, "Full Booking", []string{"Seat Selection"})
		assert.True(t, found)
		assert.Equal(t, "Booking with Seat", wf.Name)
	})

	t.Run("no match", func(t *testing.T) {
		_, found := FindComposedWorkflow(g, "Full Booking", []string{"Nonexistent"})
		assert.False(t, found)
	})

	t.Run("empty addons", func(t *testing.T) {
		_, found := FindComposedWorkflow(g, "Full Booking", nil)
		assert.False(t, found)
	})
