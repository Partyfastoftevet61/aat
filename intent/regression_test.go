...
func TestInterpret_Regression(t *testing.T) {
	tests := []regressionCase{
		{
			name:       "booking_basic",
			graphPath:  "../graph/testdata/valid/airline_booking.yaml",
			fixtureDir: "booking_basic",
			prompt:     "book a flight from DEN to SFO",
			expect: regressionExpect{
				goalNode:     "commitBooking",
				stepNodes:    []string{"searchFlights", "createItinerary", "addOffer", "addTraveler", "commitBooking"},
				goalStep:     "commitBooking",
				cleanupNodes: []string{"ignoreItinerary"},
				deps: map[string][]string{
					"addOffer":      {"createItinerary", "searchFlights"},
					"addTraveler":   {"createItinerary"},
...
`,
		...
		},
		{
			...
			expect: regressionExpect{
				...
				cleanupNodes: nil, // no cleanup for search-only (searchFlights has no cleanup in airline_booking)
				hardCount:    2,
				softCount:    0,
				freeCount:    4,
			},
