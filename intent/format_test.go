...
func TestFormatGraph_Airline(t *testing.T) {
	...
	assert.Contains(t, result, "Cleanup: ignoreItinerary")
}
...
func TestFormatChainResult_Airline(t *testing.T) {
	g := loadAirlineGraph(t)
	...
	assert.Contains(t, result, "searchFlights → createItinerary → addOffer → addTraveler → commitBooking")

	// Should show entry nodes
	assert.Contains(t, result, "searchFlights, createItinerary")
