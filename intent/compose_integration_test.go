...
func TestCompose_FullWithSeatSelection(t *testing.T) {
	...
	for _, w := range g.Workflows {
		if w.Name == "Full Booking with Seat Selection" {
			wf = w
			break
		}
	}
	...
	// Verify PLACEHOLDER auto-wiring: itineraryId should be wired from parent.
	wbValue, hasWB := seatSearch.Values["itineraryId"]
	require.True(t, hasWB, "itineraryId should exist in seat search values")
	assert.NotEqual(t, "PLACEHOLDER", wbValue.Default, "itineraryId should not be PLACEHOLDER")
	assert.NotEmpty(t, wbValue.From, "itineraryId should have a from reference")
	t.Logf("itineraryId wired to: %s", wbValue.From)
...
func TestCompose_FullWithAncillaries(t *testing.T) {
	...
	var wf graph.Workflow
	for _, w := range g.Workflows {
		if w.Name == "Full Booking with Ancillaries" {
			wf = w
			break
		}
	}
...
func TestCompose_FullWithSeatAndAncillaries(t *testing.T) {
	...
	var wf graph.Workflow
	for _, w := range g.Workflows {
		if w.Name == "Full Booking with Seat and Ancillaries" {
			wf = w
			break
		}
	}
...
func TestCompose_FindComposedWorkflow_Lookup(t *testing.T) {
	...
	tests := []struct {
		...
		{[]string{"Seat Selection"}, "Full Booking with Seat Selection"},
		{[]string{"Ancillary Booking"}, "Full Booking with Ancillaries"},
		{[]string{"Seat Selection", "Ancillary Booking"}, "Full Booking with Seat and Ancillaries"},
	}

	for _, tt := range tests {
		t.Run(tt.expected, func(t *testing.T) {
			wf, found := FindComposedWorkflow(g, "Full Booking", tt.addons)
