...
func TestFindWorkflowTemplate_CaseInsensitive(t *testing.T) {
	g := &graph.Graph{
		Workflows: []graph.Workflow{
			{Name: "Full Booking", Template: "workflows/booking.yaml"},
		},
	}

	path, found := FindWorkflowTemplate(g, "full booking")
	assert.True(t, found)
	assert.Equal(t, "workflows/booking.yaml", path)
}
...
func TestLoadWorkflowTemplate_Success(t *testing.T) {
	// Use the existing airline roundtrip-journey.yaml as a real template.
	g, err := graph.ParseFile("../airline/graph.yaml")
	require.NoError(t, err)

	p, err := LoadWorkflowTemplate("workflows/roundtrip-journey.yaml", "../airline", g)
	require.NoError(t, err)
	require.NotNil(t, p)
...
func TestLoadWorkflowTemplate_AbsolutePath(t *testing.T) {
	...
	absPath, err := filepath.Abs("../airline/workflows/roundtrip-journey.yaml")
	require.NoError(t, err)
...
func TestExpandMultiplicity_SingleExpansion(t *testing.T) {
	...
	// Should now have 5 steps: createItinerary, addTraveler_1, addTraveler_2, addTraveler_3, commit
...
func TestExpandMultiplicity_ClearsLiterals(t *testing.T) {
			...
			Steps: []plan.Step{
				{
					Node: "addTraveler",
					Values: map[string]plan.StepValue{
						"itineraryId": {From: "wb.itineraryId"},
						"givenName":   {Default: "Test"},
						"surname":     {Default: "Traveler"},
					},
				},
	...
	assert.Equal(t, "wb.itineraryId", second.Values["itineraryId"].From)
}
...
func TestExpandMultiplicity_PreservesWiring(t *testing.T) {
	...
	for _, step := range p.Execution.Steps {
		assert.Equal(t, "wb.itineraryId", step.Values["itineraryId"].From)
...
func TestMergeLLMValuesWithIDs_MatchesByStepID(t *testing.T) {
	...
	assert.Equal(t, "wb.id", skeleton.Execution.Steps[0].Values["itineraryId"].From)
	assert.Equal(t, "wb.id", skeleton.Execution.Steps[1].Values["itineraryId"].From)
}
...
func TestMergeLLMValuesWithIDs_RejectsHallucinatedLiterals(t *testing.T) {
	...
	assert.Equal(t, "createItinerary.itineraryId", skeleton.Execution.Steps[0].Values["itineraryId"].From)
}
