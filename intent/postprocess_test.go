...
func TestFixDependsOn_AddsMissingFromRefs(t *testing.T) {
	...
	// addOffer should depend on searchFlights and createItinerary (from refs)
	...
	// commitBooking should depend on addOffer, addTraveler, createItinerary (from refs)
	...
	p := &plan.Plan{
		Execution: plan.Execution{
			Steps: []plan.Step{
				...
				{Node: "addTraveler", Values: map[string]plan.StepValue{"surname": {Default: "Smith"}, "givenName": {Default: "John"}, "itineraryId": {From: "createItinerary.itineraryId"}}},
	...
	p := &plan.Plan{
		Execution: plan.Execution{
			Steps: []plan.Step{
				...
				{ID: "searchOnwardLeg2", Node: "searchReturnFlights"},
				{Node: "priceOfferFull"},
			},
		},
		Intent: plan.Intent{
