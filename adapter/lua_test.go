...
// --- Airline-style GDS reference resolution ---

func TestRunTransform_GDSReferenceResolution(t *testing.T) {
	// Simulates GDS response where FlightOffers contain
	// productRef strings that reference Products in a RefList.
	// RefList is a sibling of FlightOffers (not nested).
	...
	script := `
		local refLists = json_path("FlightOffersResponse.RefList")
		if not refLists then return outputs end

		local products = {}
		local flights = {}
		for _, refList in ipairs(refLists) do
			if refList["@type"] == "RefListProduct" and refList.Product then
				for _, prod in ipairs(refList.Product) do
					if prod.id then products[prod.id] = prod end
				end
			elseif refList["@type"] == "RefListFlight" and refList.Flight then
		...
		if outputs.flightOffers then
			for _, offering in ipairs(outputs.flightOffers) do
	...
	offerings := result["flightOffers"].([]any)
...
func TestRunTransform_GDS_PreservesExistingValues(t *testing.T) {
	// When inline values already exist (full mode), the transform
	...
	script := `
		...
		for _, rl in ipairs(refLists) do
			if rl["@type"] == "RefListProduct" and rl.Product then
		...
		if outputs.flightOffers then
			for _, off in ipairs(outputs.flightOffers) do
	...
	offer := result["flightOffers"].([]any)[0].(map[string]any)
