...
func TestExpandConditionalBlocks(t *testing.T) {
	tests := []struct {
		...
		{
			name: "realistic SearchModifiers",
			tmpl: `{
  "offersPerPage": 15,
  {{?carrierPreference}}"SearchModifiers": {
  ...
  "PassengerCriteria": []
		...
		{
			name: "realistic SearchModifiers absent",
			tmpl: `{
  "offersPerPage": 15,
  {{?carrierPreference}}"SearchModifiers": {
...
func TestCompoundConditional_SearchModifiers(t *testing.T) {
	body := `{
  "offersPerPage": 15,
  ...
  {{/cabinPreference|carrierPreference}}"PassengerCriteria": []
	...
	tests := []struct {
	...
	}{
		{
			...
			inputs: map[string]any{},
			check: func(t *testing.T, parsed map[string]any) {
				_, hasMods := parsed["SearchModifiers"]
				assert.False(t, hasMods, "SearchModifiers should be absent")
			},
		},
		{
			name:   "cabin only",
			...
			check: func(t *testing.T, parsed map[string]any) {
				mods := parsed["SearchModifiers"].(map[string]any)
...
func TestIterationIntegration_PriceOfferByRef(t *testing.T) {
	body := `{
...
func TestLoadTemplates(t *testing.T) {
	t.Run("valid directory", func(t *testing.T) {
		...
		assert.Contains(t, names, "airline.searchFlights")
		assert.Contains(t, names, "airline.getStatus")
		assert.Contains(t, names, "airline.priceOffer")
	})
...
func TestParseTemplateFile(t *testing.T) {
	t.Run("valid file", func(t *testing.T) {
		...
		require.NoError(t, err)
		assert.Equal(t, "airline.searchFlights", tmpl.Adapter)
	})
