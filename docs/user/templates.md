...
This is especially valuable when the raw JSON uses deeply nested paths. For example, a Airline offering element:

```yaml
response:
  extract:
    flightOffers:
      path: "FlightOffersResponse.FlightOffers.FlightOffer"
      fields:
        offeringId: "id"
