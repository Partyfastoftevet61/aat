...
createItinerary:
  ...
  adapter: createItinerary
  cleanup: deleteItinerary    # runs after plan completes
  inputs: [...]
  outputs: [...]
```
...
Starting from a scaffold with these nodes: `searchFlights`, `createItinerary`, `addOffer`, `addTraveler`, `commitBooking`
...
edges:
  ...
  # Itinerary ID flows to all steps that need it
  - from: createItinerary.itineraryId
    to: addOffer.itineraryId
  - from: createItinerary.itineraryId
    to: addTraveler.itineraryId
  - from: createItinerary.itineraryId
    to: commitBooking.itineraryId
...
The Airline booking graph went through several iterations as real API quirks were discovered (see [airline-example.md](airline-example.md#airline-api-notes)).
