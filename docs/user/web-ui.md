...
On a [gRPC](grpc.md) step the button becomes **Copy as grpcurl** and builds a `grpcurl` command instead — the target without its scheme, `-plaintext` when the target said so, and metadata as `-H`. The status pill shows the code the server sent, `NOT_FOUND` rather than the 404 it maps to, coloured by that mapping; the message beside it is the server's own. Response metadata and trailers get a panel each, because a gRPC server chooses which to send a value in.
