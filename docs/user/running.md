...
```
aat run \
  --plan plans/airline_booking.yaml \
  --env environments/airline-pp.yaml \
  --graph graph/testdata/valid/airline_booking.yaml \
  --templates adapter/testdata/templates/airline/
```
...
aat: loaded environment "airline-pp"
