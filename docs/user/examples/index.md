...
Four more projects run against real, public APIs and live in their own repositories; [Real APIs](real-apis.md) describes each. Three were built against an API's live test mode and the fourth against the database itself, in a local container; every claim in each README is something a run recorded:
