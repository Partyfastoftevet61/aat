...
execution:
  steps:
    ...
    - node: createItinerary
      description: "Create reservation itinerary"

    - node: addOffer
      dependsOn: [searchFlights, createItinerary]
      description: "Add the first available offer to the itinerary"
      selections:
        catalogOffering:
          from: searchFlights.catalogOfferings
          strategy: first
...
- `ignoreItinerary` runs as cleanup regardless of success or failure
