...
**Reference:** Airline MCP server (`vendor-mcp`) informed OAS tool design and architecture patterns.
...
- OAS tools included (schema browsing, validation, examples — similar to vendor-mcp reference)
...
**Goal:** Claude Code can browse OAS schemas, validate payloads, and generate request examples — similar to vendor-mcp but wired through AAT's graph nodes.
