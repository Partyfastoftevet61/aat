...
- Pre-composed workflows removed from airline graph — dynamic composition via `ComposeWithAddons` is sufficient
...
- All intent.goal and metadata.prompt removed from airline template YAML files
...
- `airline/graph.yaml` — pre-composed workflows removed, addon After/Wire added
...
- `airline/workflows/*.yaml` — intent.goal and metadata.prompt removed
...
- Airline graph: 3 addon workflows (ancillary, seat, traveler mod), 3 pre-composed shortcuts
...
- `airline/graph.yaml` — addon markers, 3 composed workflows
...
- Added `productRef` elementField to airline booking graph with deeply nested path
...
- `graph/testdata/valid/airline_booking.yaml` — path annotations, productRef field
...
- `workflows/airline_booking.yaml` — field names instead of gjson paths
- `plan/testdata/valid/airline_booking.yaml` — same
