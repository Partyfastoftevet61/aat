...
- Manual test: `cd ../aat-airline && ../aat/aat prompt "book a flight from rome to new york"` — verify selection still works

---

## Session 2: Project-Level Prompt Extensions (aat + airline repos)

Some improvements require project-specific content that doesn't belong hardcoded in `aat`. This session adds a mechanism for projects to contribute prompt hints, then uses it in airline.
...
### 2e. Airline: annotate layers with selection hints

Update each layer YAML in `aat-airline/layers/` to add `selectionHint`:
...
**Files:** `aat-airline/layers/*.yaml`

### 2f. Airline: mark broken workflows as deprecated

In `aat-airline/graph.yaml`, add `deprecated: true` to the four broken Full slot options (lines ~67-80). Remove "(don't use, broken)" from their descriptions. Clean up the descriptions.

**Files:** `aat-airline/graph.yaml`
...
- Run `make check`
- Manual test with airline to verify layer hints appear and broken options are excluded

---

## Session 3: Few-Shot Examples (aat + airline repos)
...
### 3c. Airline: add 2-3 few-shot examples

Add examples to `aat-airline/graph.yaml`:
...
- Manual end-to-end test with airline
- Run `make check`
...
| `aat-airline/graph.yaml` | Mark deprecated, add examples (Sessions 2-3) |
| `aat-airline/layers/*.yaml` | Add selectionHint (Session 2) |
