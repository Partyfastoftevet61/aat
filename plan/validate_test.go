...
func loadAirlineGraph(t *testing.T) *graph.Graph {
	t.Helper()
	g, err := graph.ParseFile("../graph/testdata/valid/airline_booking.yaml")
	require.NoError(t, err)
	return g
}

func TestValidate_AirlineBooking_HappyPath(t *testing.T) {
	g := loadAirlineGraph(t)
	p, err := ParseFile("testdata/valid/airline_booking.yaml")
	require.NoError(t, err)
...
func TestValidate_InputWiredByEdge_NoValueNeeded(t *testing.T) {
	...
	// addOffer needs: itineraryId, catalogOfferingsId, offeringId, productRef
	...
	p := &Plan{
		Execution: Execution{
			Steps: []Step{
				...
				{Node: "createItinerary"},
				{Node: "addOffer", DependsOn: []string{"searchFlights", "createItinerary"}, Selections: map[string]StepSelection{
					"offering": {From: "searchFlights.catalogOfferings", Strategy: "first"},
				}, Values: map[string]StepValue{
	...
	t.Run("invalid runOn", func(t *testing.T) {
		p := &Plan{
			Execution: Execution{
				...
				Cleanup: []CleanupStep{
					{Node: "ignoreItinerary", RunOn: "bogus"},
				},
			},
		}
		err := Validate(p, g)
		...
		p := &Plan{
			...
			Execution: Execution{
				Steps: []Step{
					...
					{Node: "createItinerary"},
					{Node: "addOffer", DependsOn: []string{"searchFlights", "createItinerary"}, Values: map[string]StepValue{
						"itineraryId":        {From: "createItinerary.itineraryId"},
						"catalogOfferingsId": {From: "searchFlights.catalogOfferingsId"},
						"offeringId":         {Default: "o1"},
						"productRef":         {Default: "p0"},
					}},
					{Node: "addTraveler", DependsOn: []string{"createItinerary"}, Values: map[string]StepValue{
						"itineraryId": {From: "createItinerary.itineraryId"},
						"surname":     {Default: "S"},
						"givenName":   {Default: "J"},
						"birthDate":   {Default: "1990-01-15"},
					...
					{Node: "commitBooking", DependsOn: []string{"addOffer", "addTraveler", "createItinerary"}, IsGoal: true, Values: map[string]StepValue{
						"itineraryId": {From: "createItinerary.itineraryId"},
