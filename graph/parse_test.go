...
func TestParse_AirlineBooking(t *testing.T) {
	...
	require.NotNil(t, search)
	assert.Equal(t, "searchFlights", search.Name)
	assert.Equal(t, "airline.searchFlights", search.Adapter)
	assert.Equal(t, "Search for available flights via Airline catalog", search.Description)
	...
	assert.Equal(t, "airline.priceOffer", price.Adapter)
	...
	// --- createItinerary ---
	wb := g.Nodes["createItinerary"]
	require.NotNil(t, wb)
	assert.Equal(t, "airline.createItinerary", wb.Adapter)
	...
	assert.Equal(t, "airline.addOffer", addOff.Adapter)
	assert.Len(t, addOff.Inputs, 4)
	assert.Equal(t, "itineraryId", addOff.Inputs[0].Name)
	...
	assert.Equal(t, "airline.addTraveler", trav.Adapter)
	...
	assert.Equal(t, "airline.commitBooking", commit.Adapter)
	assert.Len(t, commit.Inputs, 3)
	assert.Equal(t, "itineraryId", commit.Inputs[0].Name)
	...
	// --- ignoreItinerary ---
	ignore := g.Nodes["ignoreItinerary"]
	require.NotNil(t, ignore)
	assert.Equal(t, "airline.ignoreItinerary", ignore.Adapter)
	assert.Len(t, ignore.Inputs, 1)
	assert.Equal(t, "itineraryId", ignore.Inputs[0].Name)
	...
	// createItinerary fan-out
	assert.Contains(t, edgeMap, "createItinerary.itineraryId->addOffer.itineraryId")
	assert.Contains(t, edgeMap, "createItinerary.itineraryId->addTraveler.itineraryId")
	assert.Contains(t, edgeMap, "createItinerary.itineraryId->commitBooking.itineraryId")
	assert.Contains(t, edgeMap, "createItinerary.itineraryId->ignoreItinerary.itineraryId")
