...
func TestBackwardChain_AirlineFullGraph_ConfirmItinerary(t *testing.T) {
	...
	// Note: addOfferFull → addPayment ordering was via data-flow edge
	// (offerRef), which no longer exists. No requires/satisfies
...
func TestBackwardChain_AirlineFullGraph_CommitExchangeTicket(t *testing.T) {
	...
	// createItinerary (preferred) provides itineraryCreated for addPaymentMethodCash.
	assert.Contains(t, result.Nodes, "createItinerary")
...
func TestBackwardChain_AirlineFullGraph_CommitTicket(t *testing.T) {
	...
	// addPayment requires [paymentMethodAdded]
	// addPaymentMethodCash requires [itineraryCreated]
	// createItinerary satisfies [itineraryCreated] (preferred)
