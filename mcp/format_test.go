...
func TestFormatNodeDetail(t *testing.T) {
	t.Run("full node", func(t *testing.T) {
		...
		assert.Contains(t, result, "**OAS:** operationId=CreateAirSearch (spec: airline.yaml)")
		...
		assert.Contains(t, result, "## Depended On By")
		assert.Contains(t, result, "bookFlight")
	})
