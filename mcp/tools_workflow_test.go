...
func workflowTestGraph() *graph.Graph {
	return &graph.Graph{
		...
		Nodes: map[string]*graph.Node{
			...
			"book":       {Name: "book", Adapter: "book", Inputs: []graph.Input{{Name: "flightId", Type: "string"}}, Outputs: []graph.Output{{Name: "itineraryId", Type: "string"}}},
			"commit":     {Name: "commit", Adapter: "commit", Inputs: []graph.Input{{Name: "itineraryId", Type: "string"}}, Outputs: []graph.Output{{Name: "locator", Type: "string"}}},
			"searchSeat": {Name: "searchSeat", Adapter: "searchSeat", Inputs: []graph.Input{{Name: "itineraryId", Type: "string"}}, Outputs: []graph.Output{{Name: "seats", Type: "string[]"}}},
			"addSeat":    {Name: "addSeat", Adapter: "addSeat", Inputs: []graph.Input{{Name: "itineraryId", Type: "string"}, {Name: "seatId", Type: "string"}}, Outputs: []graph.Output{{Name: "confirmId", Type: "string"}}},
		},
	}
}
...
func TestHandleListWorkflows_WithWorkflows(t *testing.T) {
	...
	assert.Contains(t, text, "Full Booking")
...
func TestHandleGetWorkflowDetail_Valid(t *testing.T) {
	...
	assert.Contains(t, text, "| itineraryId | from | book.itineraryId |")
...
func TestHandleGetWorkflowDetail_StepIDDiffersFromNode(t *testing.T) {
	...
	assert.Contains(t, text, "### 1. search")
	// Step 2 (searchOnwardLeg2): StepID != Node, should show Operation line
	assert.Contains(t, text, "### 2. searchOnwardLeg2")
