...
func TestDetectResponseEnvelope_CommonRoot(t *testing.T) {
	extract := map[string]adapter.ExtractRule{
		"offerings": {Path: "FlightOffersResponse.FlightOffers"},
		"nextStep":  {Path: "FlightOffersResponse.NextSteps"},
	}
	assert.Equal(t, "FlightOffersResponse", detectResponseEnvelope(extract))
}

func TestDetectResponseEnvelope_MixedRoots(t *testing.T) {
	extract := map[string]adapter.ExtractRule{
		"offerings": {Path: "FlightOffersResponse.data"},
...
func TestExtractTransformSummary_MultiLine(t *testing.T) {
	script := `-- Resolve GDS RefList cross-references.
-- In GDS mode, FlightOffers contain only reference strings.
local data = json.decode(raw)`
	result := extractTransformSummary(script)
	assert.Contains(t, result, "Resolve GDS RefList cross-references.")
