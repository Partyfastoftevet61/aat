...
func TestDocsGenerateCommand_SplitMode(t *testing.T) {
	...
	for _, name := range []string{"searchFlights", "priceOffer", "createItinerary", "addOffer", "addTraveler", "commitBooking", "ignoreItinerary"} {
...
func TestDocsGenerateCommand_CustomTitle(t *testing.T) {
	...
	args := &docsArgs{
		...
		NodeDocsDir: filepath.Join(tmp, "nonexistent-docs"),
		OutputPath:  outputPath,
		Title:       "Airline Booking Workflow",
	}
	...
	assert.Contains(t, string(content), "# Airline Booking Workflow")
}
