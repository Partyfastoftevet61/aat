...
| `{{name}}`, `{{name + 3 days}}`, `{{name - 5}}` | Another input of the same step, declared earlier on the node. A number offset adds to a number (decimal text keeps its places); a unit offset moves Unix seconds by the unit or a `YYYY-MM-DD` date by whole days |
| `{{step.output}}`, `{{step.output - 500}}` | An earlier main step's output, such as `{{checkout.total}}`, in step values, assertions, `repeat.until`, and selection filters, with the same offsets; `from:` copies an output as it is |
