...
func TestFindPlanStepYAML_MatchCleanupStep(t *testing.T) {
	...
	a.Cleanup = []archive.StepRecord{makeStep("cancelItinerary", 200, 50)}
	a.Metadata.Plan = &plan.Plan{
		Execution: plan.Execution{
			Steps: []plan.Step{
				{Node: "search"},
			...
			Cleanup: []plan.CleanupStep{
				{Node: "cancelItinerary", RunOn: "always"},
			},
		},
	}
	result := findPlanStepYAML(a, "cancelItinerary", "cancelItinerary", true)
	assert.Contains(t, result, "node: cancelItinerary")
