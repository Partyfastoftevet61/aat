...
visualizers:
  - id: flight-search
    ...
    match:
      bodyContains: FlightOffersResponse
  - id: reservation
...
`)
	...
	assert.Equal(t, "FlightOffersResponse", defs[0].Match.BodyContains)
...
func TestMatchVisualizers_BodyContains(t *testing.T) {
	defs := []VisualizerDef{
		{ID: "flight", Name: "Flight", Match: VisualizerMatch{BodyContains: "FlightOffersResponse"}},
		{ID: "hotel", Name: "Hotel", Match: VisualizerMatch{BodyContains: "HotelResponse"}},
	}

	body := json.RawMessage(`{"FlightOffersResponse":{"offers":[]},"transactionId":"abc"}`)
...
func TestMatchVisualizers_CombinedMatch(t *testing.T) {
	...
	body := json.RawMessage(`{"FlightOffersResponse":{}}`)

	// Both match
	hits := matchVisualizers(defs, "FlightOffers", body)
	require.Len(t, hits, 1)
	...
	// Body doesn't match
	hits = matchVisualizers(defs, "FlightOffers", json.RawMessage(`{"Other":{}}`))
	assert.Empty(t, hits)
}
...
func TestEnrichStepVisualizers(t *testing.T) {
	s := &Server{
		visualizers: []VisualizerDef{
			{ID: "flight", Name: "Flight Search", Match: VisualizerMatch{BodyContains: "FlightOffersResponse"}},
		},
	}
	...
	step := &StepDetail{
		...
		Response: &ResponseDetail{
			Status: 200,
			Body:   json.RawMessage(`{"FlightOffersResponse":{"offers":[]}}`),
		},
	}
...
`)
	...
	a.Steps[0].Response.Body = json.RawMessage(`{"FlightOffersResponse":{"offers":[]}}`)
